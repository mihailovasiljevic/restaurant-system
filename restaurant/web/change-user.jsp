...
<html>
	...
	<body>
        ...
        <c:redirect url="http://localhost:8080/restaurant/login.jsp" />
		</c:if>

    <c:choose>
        <c:when test="${sessionScope.user.userType.name eq 'GUEST'}">
           <c:redirect url="http://localhost:8080/restaurant/guest.jsp" />
        </c:when>
        <c:when test="${sessionScope.user.userType.name eq 'SYSTEM_MENAGER'}">
            <!-- Forma za izmenu podataka menadzera sistema -->
                <div class="container">
        ...
        <c:otherwise>
          <c:redirect url="http://localhost:8080/restaurant/restaurant-menager.jsp" />
        </c:otherwise>
    </c:choose>
